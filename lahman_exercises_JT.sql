...
SELECT 
	...
	ROUND(AVG(so::NUMERIC / g), 2) AS average_strikeouts_per_game,
	ROUND(AVG(hr::NUMERIC / g), 2) AS average_home_runs_per_game
FROM teams
WHERE yearid >= 1920
GROUP BY decade
ORDER BY decade;
...
SELECT 
    ...
    ROUND((b.sb::numeric / (b.sb + b.cs)) * 100, 2) AS stolen_base_percentage
FROM batting b
JOIN people p
...
SELECT MAX(w) AS most_wins_without_ws 
FROM teams 
...
SELECT MIN(w) AS fewest_wins_ws_champion
FROM teams
WHERE yearid BETWEEN 1970 AND 2016
AND yearid <> 1981
AND wswin = 'Y';
